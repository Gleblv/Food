...
function calc() {
  ...
  if (localStorage.getItem("sex")) {
    // Если в локальном хранилище уже есть какие-то данные
  ...
  function initLocalSettings(selector, activeClass) {
    ...
    elements.forEach(element => {
      ...
      if (element.getAttribute("id") === localStorage.getItem("sex")) {
        // Если значение атрибута есть в локальном хранилище
  ...
  function calcTotal() {
    if (!sex || !height || !weight || !age || !ratio) {
      // Если не указаны какие-то данные
    ...
    if (sex == "female") {
      // Расчитываем кол-во кл в зависимости от пола
  ...
  function getSaticInf(selector, activeClass) {
    // функция для получения информации
    const elements = document.querySelectorAll(selector);
    elements.forEach(element => {
      // навешиваем обработчик событий на каждый элемент
      element.addEventListener("click", e => {
        if (e.target.getAttribute("data-ratio")) {
          // если у элемента есть атрибут со значением data-ratio то
          ...
          // в остальных случаях получаем id элемента
  ...
  function getDinamicInf(selector) {
    // функция для получения информации из input
    ...
    input.addEventListener("input", () => {
      ...
      switch (input.getAttribute("id")) {
        // определяем в какую переменную записывать значение input
        case "height":
          height = +input.value;
          break;
...
function cards() {
  ...
  class Card {
    // Создаём класс
    constructor(src, alt, menuTitle, text, price, parentSelector) {
      // "...clases" - массив с аргументы, которые мы в будущем можем доваить, а можем и нет
      this.src = src;
      this.alt = alt;
    ...
    cahngeToUAH() {
      // Метод для конвертации USD в UAH
      this.price *= this.transfer;
    }

    render() {
      // Метод для создания элемента и помещения его на страницу
      const element = document.createElement("div"); // Создаём контейнер

      if (this.clases.length === 0) {
        // Если в массив "clases" не были помещены аргументы, то добавляем один по умолчанию
  ...
  .then(data => {
    data.forEach(_ref => {
      ...
      // Перебираем массив с объектами с сервера и берём из объектов свойства
...
function forms(formSelector, modalTimeID) {
  ...
  function bindPostData(form) {
    // Создаём функцию для отпрвки формы
  ...
  function showThanksModal(message) {
    ...
    setTimeout(() => {
      // Сбрасываем новый контент и возвращаем старый
      thanksModal.remove();
...
function modal(trigerSelector, modalSelector, modalTimeID) {
  ...
  modalTrigers.forEach(item => {
    // Навешиваем обработчик событий на кнопки из псевдомассива
    item.addEventListener("click", () => openModal(modalSelector, modalTimeID));
  });
  modal.addEventListener("click", event => {
    // Закрытие посредством нажатия на пустую область
    if (event.target === modal || event.target.getAttribute("data-close") == "") {
      // Если точка куда кликнул пользователь совпадает с точкой вне модельного окна, то закрываем его
      closeModal(modalSelector);
    }
  });
  document.addEventListener("keydown", event => {
    // Закрытие посредством нажатия кнопки Escape
    if (event.code === "Escape" && modal.classList.contains("show")) {
      // Если код кнопки равен Escape и модельное окно показано, то закрываем его
      closeModal(modalSelector);
    }
  });

  function openModalByScroll() {
    // Открываем модельное окно когда пользователь долистывает страничку до конца
    if (window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight - 1) {
      // Если высота пролистанного контента + высота окна с которым работает пользователь равна высоте всего сайта, то показываем модельное окно
...
function slider(_ref) {
  ...
  const sliderContainers = document.querySelectorAll(slide),
        ...
        // своеобразное окно через которое будет видно только один элемент
  ...
  sliderContainers.forEach(slide => {
    // делаем все элементы одинаково ширины
    slide.style.width = width;
  });
  slider.style.position = "relative";
  const indicators = document.createElement("ol"),
        // создаём список в который помещаем точки
  ...
  for (let i = 0; i < sliderContainers.length; i++) {
    // с помощью цикла создаём кол-во точек = кол-ву слайдов
  ...
  rightSlideBtn.addEventListener('click', () => {
    if (offset == toNum(width) * (sliderContainers.length - 1)) {
      // Если мы отступ доходит до конца, то возвращаем всё в начало
      offset = 0;
    } else {
      // в отсальных случаях отступаем вправо на ширину одного элемента
      offset += toNum(width);
    }
  ...
  leftSlideBtn.addEventListener('click', () => {
    if (offset == 0) {
      // если мы находимся в начале то отступаем в конец
      offset = toNum(width) * (sliderContainers.length - 1);
    } else {
      // в остальных случаем отступаем влево на ширину одного элемента
      offset -= toNum(width);
    }
  ...
  tabsMenu.addEventListener("click", event => {
    ...
    if (target && target.classList.contains(tabsSelector.slice(1))) {
      tabs.forEach((item, i) => {
        // Перебираем все табы
        if (item == target) {
          // Если один из табов совпадает со значение target, то показываем его
          hideTabContent();
          showTabContent(i);
        }
...
function timer(id, deadline) {
  function getTimeRemaning(endtime) {
    // Получаем разницу между сегодняшней датой и датой окончания и высчитываем из этой разницы кол-во дней, часов и т.д.
  ...
  function getZero(num) {
    // Если число однозначное - добавляем 0
    if (num >= 0 && num < 10) {
      return `0${num}`;
    } else {
      return num;
...
const postData = async (url, data) => {
  // оператор async говорит что внутри функции у нас будет какой-то асинхронный код
  const res = await fetch(url, {
    // await ставиться перед операциями которых необходимо дождаться
    method: "POST",
    //метод
    headers: {
      //заголовки
...
const getResource = async url => {
  ...
  if (!res.ok) {
    // Если наш запрос не выполняется, то выводим ошибку
